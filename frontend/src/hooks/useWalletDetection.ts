...
type MnLaceOld = {
  ...
  serviceUriConfig: () => Promise<{ networkId?: string; substrateNodeUri?: string }>;
};
...
export function useWalletDetection(): UseWalletDetectionReturn {
  ...
  const connectDemo = useCallback(() => {
    ...
    setWalletInfo({
      ...
      networkId: import.meta.env.VITE_NETWORK || "preprod",
  ...
  const connect = useCallback(async () => {
    ...
    try {
      if ("connect" in lace && typeof lace.connect === "function") {
        ...
        const desiredNetwork = import.meta.env.VITE_NETWORK || 'preprod';
        ...
        if (!connected) {
          console.warn("[Demo Mode]: All wallet connections failed, falling back to demo wallet");
          connectDemo();
          return;
        }
        ...
        try {
          const addr = await withTimeout(connected.getUnshieldedAddress(), SYNC_TIMEOUT_MS);
          address = addr.unshieldedAddress;
        } catch (e: unknown) {
          console.warn("[Wallet Sync Warning]: Proceeding with partial sync —", e instanceof Error ? e.message : e);
        }
        try {
          const config = await withTimeout(connected.getConfiguration(), SYNC_TIMEOUT_MS);
          networkId = config.networkId;
        } catch (e: unknown) {
          console.warn("[Wallet Sync Warning]: Could not fetch config, using desired network —", e instanceof Error ? e.message : e);
        }
      ...
      if ("enable" in lace && typeof (lace as MnLaceOld).enable === "function") {
        const oldApi = lace as MnLaceOld;
        const alreadyEnabled = await withTimeout(oldApi.isEnabled(), DEMO_TIMEOUT_MS).catch(() => false);
        if (!alreadyEnabled) {
          await withTimeout(oldApi.enable(), DEMO_TIMEOUT_MS);
        }

        let address = "";
        let networkId = import.meta.env.VITE_NETWORK || "unknown";
        try {
        ...
        try {
          const config = await withTimeout(oldApi.serviceUriConfig(), SYNC_TIMEOUT_MS);
          networkId = config.networkId || networkId;
        } catch (e: unknown) {
          console.warn("[Wallet Sync Warning]: Could not fetch service config —", e instanceof Error ? e.message : e);
        }
      ...
      console.warn("[Demo Mode]: Wallet API not recognized, falling back to demo wallet");
      connectDemo();
      return;
    } catch (err: unknown) {
      if (!mountedRef.current) return;
      console.warn("[Demo Mode]: Connection failed, falling back to demo wallet —", err instanceof Error ? err.message : err);
      connectDemo();
    }
  }, [connectDemo]);
